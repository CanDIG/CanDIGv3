...
echo ">> waiting for candig-api to start"
ingest=$(docker ps --format "{{.Names}}" | grep candig-api_1)
while [ $? -ne 0 ]
do
  echo "..."
  sleep 1
...
bash $PWD/create_service_store.sh "candig-api"
...
if [[ $CANDIG_SITE_ADMIN_USER != "" ]]; then
  bash $PWD/exec_with_expected.sh "curl -si \"${CANDIG_URL}/candig-api/v1/authz/service-info\" -H \"Authorization: Bearer ${site_admin_token}\"" "200 OK"
  while [ $? -ne 0 ]
  do
    echo "..."
      sleep 2
  ...
  bash $PWD/exec_with_expected.sh "curl -sX \"POST\" \"${CANDIG_URL}/candig-api/v1/authz/user/pending/${CANDIG_SITE_ADMIN_USER}\" -H \"Authorization: Bearer ${site_admin_token}\"" "$CANDIG_SITE_ADMIN_USER"
fi
