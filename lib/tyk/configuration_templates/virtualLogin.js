...
var loginHelper = {
    ...
    loginRedirect: function(spec, app) {
        ...
        url += '/realms/' + spec.config_data.KEYCLOAK_REALM
    ...
    getOIDCToken: function(body, spec) {
        tokenRequest = {
            ...
            "Resource": "/realms/" + spec.config_data.KEYCLOAK_REALM + "/protocol/openid-connect/token"
        }
...
function loginHandler(request, session, spec) {
    ...
    if (code === undefined) {
        ...
        if (_.has(decodedBody, "access_token") && _.has(decodedBody, "refresh_token")) {
            var responseObject = {
                Body: "Success",
                Headers: {
                    "Authorization": "Bearer " + decodedBody["access_token"],
                    "Set-Cookie": loginHelper.setCookie(decodedBody["refresh_token"], spec),
