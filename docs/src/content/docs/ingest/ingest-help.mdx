...
If validation fails, it is important to fix the issues. The validation should give an error message that gives you enough information to fix the problem. If you are still struggling then [reach out the CanDIG team](https://github.com/CanDIG/CanDIGv3-ingest/issues/new/choose) and provide the error message so that we can try to help you. If all else fails, it is possible to ingest without the failing values and reingest the data with valid values at a later point.
