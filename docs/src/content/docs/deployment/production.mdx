...
3. Get a site admin token using the default site admin user to login to the data portal

4. Set the role of the real user to a site admin with the following curl command:

```bash
curl -X POST $CANDIG_URL'/candig-api/v1/authz/site-role/admin/user_id/<YOUR-EMAIL-ADDRESS>' -H 'Authorization: Bearer '$TOKEN
```

5. Check the role assignment was successful by verifying the following command returns `True`:

```bash
curl -X GET $CANDIG_URL'/candig-api/v1/authz/site-role/admin/user_id/<YOUR-EMAIL-ADDRESS>' -H 'Authorization: Bearer '$TOKEN
```
