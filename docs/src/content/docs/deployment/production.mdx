...
banner:
  content: We are yet to confirm production settings for CanDIGv3, please reach out before using for production data 
---
...
:::danger
We are yet to confirm production settings for CanDIGv3, please reach out before using for production data
:::
...
Specifically, the UHN proxy forwards all candig.uhnresearch.ca and candigauth.uhnresearch.ca requests (port 443) to candig1:5080 (tyk) and candig1:8080 (keycloak) respectively, thereby acting as a firewall.  All CanDIGv3 microservices can only be accessed through Tyk.

:::note
BCGSC initially had an issue with having a double proxy which caused a double URL bug. Switching to a single proxy resolved this issue. Please [reach out](https://github.com/CanDIG/CanDIGv3/issues/new/choose) if you need help solving this.
:::

:::note
It is essential to properly setup the reverse proxy to set the `X-Forwarded-*` headers (or the `Forwarded` header if you must) along with the `KEYCLOAK_PROXY_HEADERS` `.env` variable, or very strange errors may occur. For example, if you run your reverse proxy on the same server as CanDIG and do not properly set X-Forwarded-Host, the OIDC connections will throw errors about being unable to connect to localhost.
:::
...
The following default settings in the `.env` file should be changed when deploying CanDIG in a production environment. You can also take a look at the [`example-production.env`](https://github.com/CanDIG/CanDIGv3/blob/develop/etc/env/example-production.env) and [`etc/env/production-diff-template.diff`](https://github.com/CanDIG/CanDIGv3/blob/develop/etc/env/production-diff-template.diff) files with the changes below incorporated.
...
## Setting Site Logo
To customize the site logo, you need to place your image in the candig-data-portal either before building or within the container after running the build-all or install-all commands. The image should be located at `CanDIGv3/lib/candig-data-portal/candig-data-portal/src/assets/images/users/siteLogo.png`. This will overwrite the default logo.
...
```bash
 ...
 ```bash
 cp your_image_path/siteLogo.png CanDIGv3/lib/candig-data-portal/candig-data-portal/src/assets/images/users/siteLogo.png
 ```
...
```bash
curl -X POST $CANDIG_URL'/ingest/site-role/admin/user_id/<YOUR-EMAIL-ADDRESS>' -H 'Authorization: Bearer '$TOKEN
```

5. Check the role assignment was successful by verifying the following command returns `True`:

```bash
curl -X GET $CANDIG_URL'/ingest/site-role/admin/user_id/<YOUR-EMAIL-ADDRESS>' -H 'Authorization: Bearer '$TOKEN
```
...
Comment out the value of DEFAULT_SITE_ADMIN_USER in your .env file:
```
