...
As new stable versions of the CanDIGv3 stack are released, it is recommended that production deployments be updated as soon as possible as deployments at different versions are unlikely to be able to be federated.
...
1. Backup your existing deployment using the steps on the page: [Backing up and restoring CanDIG data](backup-restore-candig). This should be done regardless of wheteher you think the deployment will destroy any data, just in case anything goes wrong. Things you will want to back up are:
    ...
    - git repos diffs - see expandable section below:
        <details>
        <summary>Example bash script of how to save git diffs of all CanDIGv3 repos</summary>
        ```bash
        #!/bin/bash
...
2. Pull the latest stable release from the CanDIGv3 repo and update submodules
...
9. Run integration tests to ensure all the services are running as expected
    ...
    <summary> Running integration tests when default site admin is removed </summary>
        ...
        1. To reinstate `site_admin@test.ca` as a site admin user, from the CanDIGv3 repo in a terminal 
    ...
    If tests fail, check out the [troubleshooting section](stack-troubleshooting) or [reach out for help](https://github.com/CanDIG/CanDIGv3/issues/new/choose).
