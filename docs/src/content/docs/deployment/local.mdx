...
We have successfully run and installed the CanDIGv3 stack on VMs with 4 CPUs and 8GB of memory.
...
CanDIG can be installed and deployed as below for development situations where no real data will ever be ingested into the system. For critical differences in production deployments, please see the <a href="/CanDIGv3/deployment/production/">Guide to CanDIG production deployments</a>.

## Initialize CanDIGv3 Repo
...
```bash
git clone -b develop https://github.com/CanDIG/CanDIGv3.git
cd CanDIGv3
...
# change OS
VENV_OS=<your OS>
# set DB backup to true when first running
LOAD_DB_BACKUP=true
```
...
### 3. Download the omop database backup so that it can be loaded into postgres on startup of `candig-api`

  1. Contact a CanDIG dev to get access to the backup file called `VOCAB_bak.dump`
  2. Put file in the `CanDIGv3/lib/candig-api/candig-api/db`

:::note
Once you have loaded the backup the first time, you can set the variable `LOAD_DB_BACKUP` to `false`, so that if you need to recompose `candig-api` it doesn't re-load the backup. If you do destroy to postgres db you will need to set the variable back to `true`.
:::

### 4.a: install miniconda and initialize candig virtualenv

Use this option for systems installations. It installs miniconda in the CanDIGv3 repo.
...
### 4.b: Use an existing Conda installation
...
### 5. Activate the candig virtualenv. It may be necessary to restart your shell before doing this
...
```
  CANDIG_MODULES=logging keycloak vault redis tyk opa postgres drs candig-api htsget rnaget federation candig-data-portal
```
...
127.0.0.1 candig.docker.internal  # only if you need ipv4
```
